...
data ConDesc (is : Telescope iₙ) : ℕ → Set₁ where
  ...
  ×'   : ⟦ is ⟧telD → ConDesc is aₙ → ConDesc is (suc aₙ)
...
⟦_⟧c : ConDesc is aₙ → (⟦ is ⟧telD → Set) → ⟦ is ⟧telD → Set
⟦ one' d ⟧c  X t = d ≡ t
⟦ Σ' S D ⟧c  X t = Σ[ s ∈ S ] (⟦ D s ⟧c X t)
⟦ ×' d D ⟧c  X t = X d × ⟦ D ⟧c X t

⟦_⟧ : DataDesc is cₙ → (⟦ is ⟧telD → Set) → ⟦ is ⟧telD → Set
⟦_⟧ {cₙ = cₙ} D X t = Σ[ i ∈ Fin cₙ ] (⟦ proj₂ (D i) ⟧c X t)

-- fix point
data μ (D : DataDesc is cₙ) : ⟦ is ⟧telD → Set where 
...
conᵢ : {D : DataDesc is cₙ}{d : ⟦ is ⟧telD} → μ D d → Fin cₙ
conᵢ ⟨ x ⟩ = proj₁ x 

-- gets number of constructor elements from fix point
conₙ : {D : DataDesc is cₙ}{d : ⟦ is ⟧telD} → μ D d → ℕ
conₙ {D = D} ⟨ i , _ ⟩ = proj₁ (D i)


-- collects every subobject in D
AllC : (C : ConDesc is aₙ)(X : ⟦ is ⟧telD → Set) 
...
All :  (D : DataDesc is cₙ)(X : ⟦ is ⟧telD → Set) 
...
elim-μ : (D : DataDesc is cₙ)(P : (d : ⟦ is ⟧telD) → μ D d → Set ℓ)  
    → (m : (d : ⟦ is ⟧telD) → (x : ⟦ D ⟧ (μ D) d) → All D (μ D) P d x → P d ⟨ x ⟩)
    → (d : ⟦ is ⟧telD) → (x : μ D d) → P d x 
elim-μ {is = is} {cₙ = cₙ} D P m d ⟨ x ⟩ = m d x (all D D P m d x) where 
